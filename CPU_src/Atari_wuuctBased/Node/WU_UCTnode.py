...
class WU_UCTnode():
    ...
    def update_history(self, idx, action_taken, reward):
        if idx in self.traverse_history:
            ...
            # print("action idx",idx,"is pushed into traverse_history")
            return True

    # Incomplete update, called by WU_UCT.py
    def update_incomplete(self, idx):
        # print("In update_incomplete, idx:",idx)
        # print("In update_incomplete, self.traverse_history:",self.traverse_history)
        # brute fore:
    ...
    def update_complete(self, idx, accu_reward):
        if idx not in self.traverse_history:
            ...
            # print("action idx",idx,"is popped from traverse_history")
            action_taken = item[0]
            reward = item[1]
