...
class PoolManager():
    ...
    def assign_expsim_task(self, checkpoint_data, curr_node,
                              saving_idx, task_simulation_idx, command #added
                              ):
        worker_idx = self.find_idle_worker()

        self.send_safe_protocol(worker_idx, command, ( #command can be "expsim" or "onlysim"
            checkpoint_data,
            curr_node,
            saving_idx,
            task_simulation_idx
    ...
    def get_complete_expsim_task(self):
        flag = False
        selected_worker_idx = -1
        ...
        command, args = item
        assert command == "ReturnExpSim" or "ReturnSim" or "ReturnExpNoSim"
