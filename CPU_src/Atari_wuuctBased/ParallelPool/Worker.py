...
class Worker(Process):
    ...
    def run(self):
        ...
        while True:
            ...
            rcv_task_time=time.perf_counter()-start
            ...
            # elif command == "Expansion":
                ...
                expand_action, expand_next_state, expand_reward, expand_done, \
                    expand_checkpoint_data = self.expand_node(checkpoint_data, curr_node)

                item = (expand_action, expand_next_state, expand_reward, expand_done, expand_checkpoint_data,
                        saving_idx, task_idx)
                # content in the item used by simulation:
                # expand_action, curr_node(?)<-self.expsim_task_recorder.pop(task_idx)[2] in master process, wchih is the same curr_node in line 64
                # saving_idx, expand_next_state
                if (expand_done): # If this expansion result in a terminal node, perform update directly. (simulation is not needed)
                    self.send_safe_protocol("ReturnExpNoSim", item)
                else: #simulation
                    # task_idx=task_idx
                    # checkpoint_data=checkpoint_data #state before expansion action
                    first_action=expand_action
                    ...
                    if first_action is not None:
                        # first 3 args are for node insertion after expansion
                        self.send_safe_protocol("ReturnExpSim", (expand_action,expand_next_state,saving_idx,checkpoint_data, task_idx, accu_reward, reward, done))
                    else:
                        self.send_safe_protocol("ReturnExpSim", (expand_action,expand_next_state,saving_idx,checkpoint_data, task_idx, accu_reward, prior_prob))
                                           
                # self.send_safe_protocol("ReturnExpansion", item)

            elif command == "onlysim":
                # no expansion, so no need for first_action
                if args is None:
                    raise RuntimeError

                # checkpoint_data, curr_node, saving_idx, task_idx = args
                # checkpoint_data, first_action, global_idx, task_idx,
                checkpoint_data, first_action, saving_idx, task_idx = args
                ...
                if first_action is not None:
                    self.send_safe_protocol("ReturnSim", (task_idx, accu_reward, reward, done))
                else:
                    self.send_safe_protocol("ReturnSim", (task_idx, accu_reward, prior_prob))
            expsimtime=time.perf_counter()-start
            # print("rcv_task_time, expsimtime in seconds (1 worker simulate_single_step):",rcv_task_time, expsimtime)
    ...
    def simulate(self, state, max_simulation_step = 50, lamda = 0.5):
        step_count = 0
        accu_reward = 0.0
        accu_gamma = 1.0
