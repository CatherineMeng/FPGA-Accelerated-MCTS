...
extern "C" {
	        ...
	        )
	{
		...
		#pragma HLS STREAM variable=IDRpipe1 depth=32
	 	#pragma HLS bind_storage variable=IDRpipe1 type=fifo impl=SRL
		hls::stream<BpSel_inputObj> IDRpipe2("IDRpipe2");
		#pragma HLS STREAM variable=IDRpipe2 depth=32
	 	#pragma HLS bind_storage variable=IDRpipe2 type=fifo impl=SRL
		hls::stream<BpSel_inputObj> IDRpipe3("IDRpipe3");
		#pragma HLS STREAM variable=IDRpipe3 depth=32
	 	#pragma HLS bind_storage variable=IDRpipe3 type=fifo impl=SRL

	 	hls::stream<ap_int<1>> Wsigpipe1("Wsigpipe1");
		#pragma HLS STREAM variable=Wsigpipe1 depth=1
	 	#pragma HLS bind_storage variable=Wsigpipe1 type=fifo impl=SRL
		hls::stream<ap_int<1>> Wsigpipe2("Wsigpipe2");
		#pragma HLS STREAM variable=Wsigpipe2 depth=1
	 	#pragma HLS bind_storage variable=Wsigpipe2 type=fifo impl=SRL
		hls::stream<ap_int<1>> Wsigpipe3("Wsigpipe3");
		#pragma HLS STREAM variable=Wsigpipe3 depth=1
	 	#pragma HLS bind_storage variable=Wsigpipe3 type=fifo impl=SRL

	 	hls::stream<SelTupleOut> OutTuppipe1("OutTuppipe1");
		#pragma HLS STREAM variable=OutTuppipe1 depth=32
	 	#pragma HLS bind_storage variable=OutTuppipe1 type=fifo impl=SRL
		hls::stream<SelTupleOut> OutTuppipe2("OutTuppipe2");
		#pragma HLS STREAM variable=OutTuppipe2 depth=32
	 	#pragma HLS bind_storage variable=OutTuppipe2 type=fifo impl=SRL
		hls::stream<SelTupleOut> OutTuppipe3("OutTuppipe3");
		#pragma HLS STREAM variable=OutTuppipe3 depth=32
		...
		while (!X.empty()){
			...
			// ======back-up at root======
			inXbuf = X.read();
			dtype V = inXbuf.get_data();
			inIDbuf = ids.read();
			int id = inIDbuf.get_data();
			root.ucta[id]+=V;
			// ======selection at root======
			dtype max_uct=-100;
			int max_child=0;
		    ...
		    xbar(xbarpipe,
		    	...
		    	Wsigpipe1,Wsigpipe2,Wsigpipe3);

		    subT1_pipl(IDRpipe1,Wsigpipe1,OutTuppipe1,flushs);
		    subT1_pipl(IDRpipe2,Wsigpipe2,OutTuppipe2,flushs);
		    subT1_pipl(IDRpipe3,Wsigpipe3,OutTuppipe3,flushs);
...
void xbar(
		...
		hls::stream<ap_int<1>> &outWsig1,
		hls::stream<ap_int<1>> &outWsig2,
		hls::stream<ap_int<1>> &outWsig3
		)
{
	int cnt;
	while (1){
		...
		if (!inIDRpipe.empty() && cnt==16){
			ap_int<1> wsig=1; //break signal for while loop in the sub-tree pipeline
			outWsig1.write(wsig);
			outWsig2.write(wsig);
			outWsig3.write(wsig);
			break;
		...
		)
{
	...
	while(1){
		if (!inpipe1.empty()){
			cnt+=1;
			SelTupleOut outobj = inpipe1.read();
			// pkt128 out_buf;
	  //       // Setting data and configuration to output packet
	  //       out_buf.set_data(outobj);
	  //       out_buf.set_keep(-1);
	  //       out_buf.set_last(0);
	  //       out.write(out_buf);
			out[cnt]=outobj;
		...
		if (!inpipe2.empty()){
			cnt+=1;
			SelTupleOut outobj = inpipe2.read();
			// pkt128 out_buf;
	  //       // Setting data and configuration to output packet
	  //       out_buf.set_data(outobj);
	  //       out_buf.set_keep(-1);
	  ...
	  //       out.write(out_buf);
		...
		if (!inpipe3.empty()){
			cnt+=1;
			SelTupleOut outobj = inpipe3.read();
			// pkt128 out_buf;
	  //       // Setting data and configuration to output packet
	  //       out_buf.set_data(outobj);
	  //       out_buf.set_keep(-1);
	  ...
	  //       out.write(out_buf);
		...
		if (cnt==16 && !inpipe1.empty() && !inpipe2.empty() && !inpipe3.empty()){
			// pkt128 out_buf;
			// out_buf.set_last(1);
	  //       out.write(out_buf);
			// out[cnt]=outobj;
		...
		int flushs)
{
 	...
 	*/
  	hls::stream<BpSel_inputObj> IDRpipes[D+1];
	...
	#pragma HLS bind_storage variable=lev1 type=RAM_1P impl=bram
	#pragma HLS bind_storage variable=lev2 type=RAM_1P impl=bram
	#pragma HLS bind_storage variable=lev3 type=RAM_1P impl=uram
	#pragma HLS bind_storage variable=lev4 type=RAM_1P impl=uram
	#pragma HLS bind_storage variable=lev5 type=RAM_1P impl=uram
	...
	while(1){
		...
		else if(inIDRpipe.empty() && !inWsig.empty()){
			inWsig.read();
			break;
		}
	}
...
void subT1_lev_stage(
		...
		hls::stream<BpSel_inputObj> &IDRpipe_in, //input worker index and reward used to index memoiz_array and update(BackUp)
		hls::stream<BpSel_inputObj> &IDRpipe_out, //output worker index and reward used to index memoiz_array and update(BackUp)
		...
		)       
{
	...
	BpSel_inputObj IDR =IDRpipe_in.read();
	...
	if (!ret_signal){
		...
		newind=parindex*fo+int(parnode.numc);
	}  //returned at before last level pipeline,need to return both leaf and expanded action
	pipeobj.ind=newind;
