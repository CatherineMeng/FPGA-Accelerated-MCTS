...
#(parameter
    ...
    output wire [63:0]                   adj_list,
    output wire [31:0]                   flushs,
...
localparam
    ...
    ADDR_IER             = 6'h08,
    ADDR_ISR             = 6'h0c,
    ADDR_ADJ_LIST_DATA_0 = 6'h10,
    ADDR_ADJ_LIST_DATA_1 = 6'h14,
    ADDR_ADJ_LIST_CTRL   = 6'h18,
    ADDR_FLUSHS_DATA_0   = 6'h1c,
    ADDR_FLUSHS_CTRL     = 6'h20,
    WRIDLE               = 2'd0,
    WRDATA               = 2'd1,
    WRRESP               = 2'd2,
    WRRESET              = 2'd3,
...
//------------------------Local signal-------------------
    ...
    reg  [63:0]                   int_adj_list = 'b0;
    reg  [31:0]                   int_flushs = 'b0;
    ...
    if (ACLK_EN) begin
        if (ar_hs) begin
            ...
            case (raddr)
                ...
                ADDR_ADJ_LIST_DATA_0: begin
                    rdata <= int_adj_list[31:0];
                end
                ADDR_ADJ_LIST_DATA_1: begin
                    rdata <= int_adj_list[63:32];
                end
                ADDR_FLUSHS_DATA_0: begin
                    rdata <= int_flushs[31:0];
                end
            endcase
        end
    end
...
assign adj_list      = int_adj_list;
assign flushs        = int_flushs;
// int_event_start
always @(posedge ACLK) begin
    if (ARESET)
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_ADJ_LIST_DATA_0)
            int_adj_list[31:0] <= (WDATA[31:0] & wmask) | (int_adj_list[31:0] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        int_adj_list[63:32] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_ADJ_LIST_DATA_1)
            int_adj_list[63:32] <= (WDATA[31:0] & wmask) | (int_adj_list[63:32] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_FLUSHS_DATA_0)
            int_flushs[31:0] <= (WDATA[31:0] & wmask) | (int_flushs[31:0] & ~wmask);
    end
end
